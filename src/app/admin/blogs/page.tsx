...
export default function AdminBlogsPage() {
  ...
  const handleTranslate = async (field, from) => {
    const sourceField = field === "title" 
      ? (from === "en" ? "title" : "titleBn")
      : (from === "en" ? "content" : "contentBn");
    
    const textToTranslate = getValues(sourceField);
    if (!textToTranslate) {
      errorAlert("Text Required", "Please enter some text to translate.");
      return;
    }

    setTranslatingField(`${field}_${from}`);
    try {
      const targetLang = from === "en" ? "bn" : "en";
      const result = await translateText(textToTranslate, from, targetLang);
      const targetField = field === "title"
        ? (from === "en" ? "titleBn" : "title")
        : (from === "en" ? "contentBn" : "content");
      
      setValue(targetField, result, { shouldDirty: true, shouldValidate: true });
      successAlert("Translation Success", "Translated text filled successfully!");
    } catch (err) {
      errorAlert("Translation Failed", err.message || "Failed to auto-translate.");
    } finally {
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        ...
                        className="text-xs text-emerald-700 dark:text-emerald-400 font-semibold hover:underline flex items-center gap-1 disabled:opacity-50"
                      >
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        ...
                        disabled={translatingField === "title_bn"}
                        className="text-xs text-emerald-700 dark:text-emerald-400 font-semibold hover:underline flex items-center gap-1 disabled:opacity-50"
                      >
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        ...
                        disabled={translatingField === "content_bn"}
                        className="text-xs text-emerald-700 dark:text-emerald-400 font-semibold hover:underline flex items-center gap-1 disabled:opacity-50"
                      >
