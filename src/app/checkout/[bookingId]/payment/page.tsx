...
import { trackAddPaymentInfo, trackPurchase } from "@/lib/analytics/facebook/events";
...
const paymentSchema = z.object({
  paymentMethod: z.enum(["bkash", "nagad", "rocket", "bank"] as const, {
...
export default function PaymentPage({ params }: PaymentPageProps) {
  ...
  const hasAddedPaymentInfoRef = useRef(false);
  useEffect(() => {
    if (booking && !hasAddedPaymentInfoRef.current) {
      trackAddPaymentInfo("manual", booking.totalPrice, user || undefined);
      hasAddedPaymentInfoRef.current = true;
    }
  }, [booking, user]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      trackPurchase(purchaseBooking, booking.packageId, user || undefined);
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {errors.paymentMethod && (
                      <p className="text-xs font-medium text-destructive">{errors.paymentMethod.message?.toString()}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {errors.transactionId && (
                      <p className="text-xs font-medium text-destructive">{errors.transactionId.message?.toString()}</p>
                    )}
                  </div>
